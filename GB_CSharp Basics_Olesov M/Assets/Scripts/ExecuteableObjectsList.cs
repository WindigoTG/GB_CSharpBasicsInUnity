...
namespace BallGame
{
    public class ExecuteableObjectsList : IEnumerator, IEnumerable
    {
        private IExecuteable[] _interactiveObjects;
        private int _index = -1;

        public ExecuteableObjectsList()
        {
            var interactiveObjects = Object.FindObjectsOfType<InteractiveObject>();
            for (var i = 0; i < interactiveObjects.Length; i++)
            {
                if (interactiveObjects[i] is IExecuteable interactiveObject)
                {
                    AddExecuteableObject(interactiveObject);
                }
            }
        }
        ...
        public IExecuteable this[int index]
        {
